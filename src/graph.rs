...
pub struct HnswGraph<const D: usize> {
    ...
    /// Cached f32 representations for fast approximate distance (O2 optimization).
    /// Pre-computed during insert() to avoid repeated I64F32→f32 conversion
    /// in the two-phase search hot path. This is NOT used for consensus-critical
    /// ordering — only for pre-filtering candidates.
    #[serde(default)]
    f32_cache: Vec<Vec<f32>>,
    /// Force scalar (non-SIMD) distance computation for cross-platform
    /// verification. When true, all distance calculations bypass SIMD
    /// instructions and use pure integer arithmetic, guaranteeing identical
    /// results across x86_64, ARM64, and RISC-V.
    #[serde(default)]
...
impl<const D: usize> HnswGraph<D> {
    ...
    /// Create a new HNSW graph with force_scalar mode enabled.
    /// All distance computations will use the scalar (non-SIMD) path,
    /// enabling cross-platform hash verification.
    ...
    pub fn insert(
        ...
        if self.entry_point.is_none() {
            self.f32_cache.push(vector.to_f32_vec());
    ...
    /// Two-Phase search optimization (O2).
    ///
    /// Phase 1: Use approximate f32 distance to gather a larger candidate pool.
    /// Phase 2: Re-rank the pool using exact I64F32 distance.
    ///
    /// This reduces the number of expensive fixed-point distance computations
    /// while preserving **deterministic final ordering** (only I64F32 distances
    /// are used for the result).
    ///
    /// # Arguments
    /// * `query` - The query vector
    /// * `k` - Number of neighbors to return
    /// * `ef` - Search expansion factor (applied to Phase 1)
    /// * `phase1_multiplier` - Multiplier for candidate pool size in Phase 1
    ///   (e.g., 2 means gather 2×ef candidates approximately, verify best ef)
    pub fn search_two_phase(
        &self,
        query: &FixedPointVector<D>,
        k: usize,
        ef: usize,
        phase1_multiplier: usize,
    ) -> Result<Vec<(usize, I64F32)>> {
        if k == 0 {
            return Ok(Vec::new());
        }
        ...
        // Phase 1: Approximate search with f32 to gather larger candidate pool
        let phase1_ef = ef * phase1_multiplier.max(1);
        let approximate_candidates = self.search_layer_approximate(query, current_node, phase1_ef, 0);

        // Phase 2: Re-rank using exact I64F32 distance (deterministic)
        let mut exact_results: Vec<(usize, I64F32)> = approximate_candidates
            .into_iter()
            .filter(|&(id, _)| !self.nodes[id].deleted)
            .map(|(id, _approx_dist)| {
                let exact_dist = query.squared_distance(&self.nodes[id].vector);
                (id, exact_dist)
            })
            .collect();

        // Sort by exact distance (this is the consensus-critical ordering)
    ...
    /// Approximate search using f32 distances for Phase 1 pre-filtering.
    ///
    /// Returns (node_id, approximate_distance) pairs. This is NOT used for
    /// final ordering — only for candidate selection.
    fn search_layer_approximate(
        &self,
        query: &FixedPointVector<D>,
        entry: usize,
        ef: usize,
        ...
        let query_f32: Vec<f32> = query.to_f32_vec();

        // Use cached f32 vectors for fast distance (no I64F32→f32 conversion)
        let entry_dist = Self::approx_dist_f32_cached(&query_f32, self.get_f32_cache(entry));
        ...
        while idx < candidates.len() {
            let (current_id, current_dist) = candidates[idx];
            idx += 1;

            // Early termination: if current is further than worst result
            if results.len() >= ef {
            ...
            let neighbors = self.nodes[current_id]
                .neighbors
                .get(level as usize)
                .map(|v| v.as_slice())
                .unwrap_or(&[]);
            ...
            for &neighbor_id in neighbors {
                ...
                let dist = Self::approx_dist_f32_cached(&query_f32, self.get_f32_cache(neighbor_id));
    ...
    /// Get cached f32 vector for a node. Falls back to conversion if cache is stale.
    #[inline]
    fn get_f32_cache(&self, id: usize) -> &[f32] {
        if id < self.f32_cache.len() {
            &self.f32_cache[id]
        } else {
            // Should not happen in normal flow; fall back
            &[]
        }
    }

    /// Fast approximate f32 distance using pre-cached f32 vectors.
    /// This is the hot path for Phase 1 — pure f32 arithmetic, no conversion.
    #[inline]
    fn approx_dist_f32_cached(query_f32: &[f32], node_f32: &[f32]) -> f32 {
        let mut sum: f32 = 0.0;
        for i in 0..query_f32.len().min(node_f32.len()) {
            let diff = query_f32[i] - node_f32[i];
            sum += diff * diff;
        }
        sum
    }

    /// Fast approximate f32 distance (for Phase 1 pre-filtering only).
    /// Legacy path — converts I64F32 on-the-fly. Use approx_dist_f32_cached when possible.
    #[inline]
    #[allow(dead_code)]
    fn approx_dist_f32(query_f32: &[f32], node_vec: &FixedPointVector<D>) -> f32 {
        let mut sum: f32 = 0.0;
        for (i, &q) in query_f32.iter().enumerate() {
            let n = node_vec.components[i].to_num::<f32>();
            let diff = q - n;
            sum += diff * diff;
        }
        sum
