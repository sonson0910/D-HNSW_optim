...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig13_two_phase_search.png}
...
Before applying our optimization suite, we measure the raw overhead of I64F32 distance computation by replacing the distance function in \hnsw{} with a pure fixed-point implementation. \Cref{fig:raw_overhead} shows the results across four datasets. The raw overhead ranges from $4.4\times$ on GloVe-100 (lowest dimensionality) to $11.2\times$ on SIFT-128, confirming that the na\"ive fixed-point approach is impractical without optimization. These measurements motivate the optimization pipeline described in \Cref{sec:optimizations}, which reduces the overhead to $1.75\times$.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig12_real_i64f32_search.png}
...
\Cref{fig:determinism_table} provides a comprehensive summary of the verification results, including distance hash matching, search hash matching, and order-independence testing across all datasets and a float32 control group.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig15_determinism_verification.png}
  \caption{SHA-256 determinism verification summary. I64F32 arithmetic produces bit-identical results across all 5 runs on 3 datasets (SIFT-128, Fashion-784, GIST-960) plus a float32 control. Distance hashes, search result hashes, and order-independence tests all pass.}
  \label{fig:determinism_table}
\end{figure}
