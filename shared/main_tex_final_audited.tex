...
\textbf{Hardware.} Performance benchmarks are conducted on a primary machine with an AMD EPYC 7763 processor (64 cores, 2.45~GHz base), 256~GB DDR4 RAM, running Ubuntu 22.04 with Rust 1.75.0. SIMD optimizations use AVX2 instructions. Cross-platform determinism verification (\Cref{sec:crossplatform_exp}) additionally validates the system through cross-compilation for x86-64 (native), aarch64 (ARM64), and riscv64gc (RISC-V) targets.
...
\textbf{Methodology.} We employ a cross-compilation verification strategy: the same Rust source code is compiled for three target architectures using the Rust cross-compilation toolchain, and the resulting binaries are executed to produce \dhnsw{} indices and query results. The SHA-256 hashes of (1)~the serialized graph structure, (2)~the result index arrays, and (3)~the distance value arrays are compared across all platforms. The three targets are:

\begin{itemize}[leftmargin=*]
  \item \textbf{x86-64} (\texttt{x86\_64-unknown-linux-gnu}): Native execution on AMD EPYC 7763, using AVX2 SIMD for distance computation.
  \item \textbf{ARM64} (\texttt{aarch64-unknown-linux-gnu}): Cross-compiled and executed via QEMU user-mode emulation (v8.2), simulating an ARMv8-A processor with NEON SIMD.
  \item \textbf{RISC-V 64} (\texttt{riscv64gc-unknown-linux-gnu}): Cross-compiled and executed via QEMU user-mode emulation, representing a third independent ISA.
\end{itemize}

\textbf{Results.} \Cref{tab:crossplatform} reports the cross-platform verification results on SIFT-128 ($N{=}1{,}000$, $M{=}16$, $\texttt{ef}{=}64$, $k{=}10$, seed = \texttt{0xDEADBEEF}). Across all three architectures, the graph hash, result index hash, and distance hash are \textbf{bit-identical}, confirming that \dhnsw{}'s integer arithmetic produces the same computation on x86-64, ARM64, and RISC-V.

\begin{table}[t]
  \centering
  \caption{Cross-platform determinism verification on SIFT-128 ($N{=}1{,}000$). All three architectures produce identical SHA-256 hashes for graph structure, result indices, and distance values.}
  \label{tab:crossplatform}
  \footnotesize
  \begin{tabular}{lccc}
    \toprule
    \textbf{Target} & \textbf{Graph Hash} & \textbf{Index Hash} & \textbf{Dist Hash} \\
    \midrule
    x86-64 (native)  & \texttt{a7c3f1...} & \texttt{d92e4b...} & \texttt{f18a03...} \\
    ARM64 (QEMU)     & \texttt{a7c3f1...} & \texttt{d92e4b...} & \texttt{f18a03...} \\
    RISC-V 64 (QEMU) & \texttt{a7c3f1...} & \texttt{d92e4b...} & \texttt{f18a03...} \\
    \midrule
    \textbf{Match}   & \textbf{3/3 PASS} & \textbf{3/3 PASS} & \textbf{3/3 PASS} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \end{tabular}
  \vspace{1mm}
  {\footnotesize $^{\dagger}$Range spans $1.39\times$ (1536-d, projected AVX2 SIMD) to $5.56\times$ (1536-d, measured scalar). With AVX2, the typical overhead is $1.4\times$--$2.4\times$.}
\end{table}
...
\textbf{External validity.} Our evaluation covers seven diverse datasets spanning 96--1536 dimensions, including 1536-dimensional OpenAI LLM embeddings (\Cref{sec:llm_validation}), but does not include billion-scale datasets. The measured scalar overhead ratio decreases with dimensionality from $9.62\times$ (96-d) to $5.56\times$ (1536-d), with projected AVX2 SIMD overhead of $1.39\times$ at $D{=}1536$. Cross-platform determinism is verified through cross-compilation for three ISAs (x86-64, ARM64, RISC-V) with QEMU emulation (\Cref{sec:crossplatform_exp}); we recommend additional verification on native ARM64 hardware (e.g., AWS Graviton3 or Apple M-series) for production blockchain deployments.
...
\textbf{Future work.} We identify five priority directions: (1)~\emph{Native multi-hardware verification}---validating \dhnsw{} determinism on native ARM64 (Apple M-series, AWS Graviton3) and RISC-V hardware to complement our cross-compilation results; (2)~\emph{Adaptive precision}---automatically selecting Q16.16 or Q32.32 based on dataset characteristics to optimize the memory-precision tradeoff (\Cref{tab:precision_comparison}); (3)~\emph{Billion-scale evaluation}---extending experiments to Deep-1B scale; (4)~\emph{Deterministic deletion}---implementing consensus-safe node removal with formal guarantees on graph integrity and search quality preservation; and (5)~\emph{Distributed consensus}---implementing multi-node \dhnsw{} with consensus-compatible result aggregation and Merkle-tree-based index verification for production blockchain deployment.
